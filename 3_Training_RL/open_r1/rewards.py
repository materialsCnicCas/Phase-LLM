...
def _normalize_label(text: str) -> Optional[str]:
    """Normalize arbitrary text into one of three canonical labels."""
    if not text:
        return None
    lowered = text.lower()
...
def _extract_post_think_answer(content: str) -> str:
    """Extract answer section after </think>; fallback to full text."""
    if "</think>" in content:
        return content.split("</think>", 1)[1].strip()
    return content.strip()


def material_accuracy_reward(prompts, completions, answer, **kwargs):
    """Accuracy reward for phase prediction."""
    contents = [completion[0].get("content", "") for completion in completions]
    rewards = []

    for content, gt in zip(contents, answer):
        clean_content = _extract_post_think_answer(content)
        gt_label = _normalize_label(str(gt))
        if gt_label is not None and gt_label in clean_content:
            rewards.append(1.0)
        else:
            rewards.append(0.0)
...
def material_format_reward(prompts, completions, **kwargs):
    """Format reward for structured outputs."""
    pattern = re.compile(r"<think>.*?</think>\s*\n\n.+", flags=re.DOTALL)
    contents = [completion[0].get("content", "") for completion in completions]
    return [0.5 if pattern.match(content or "") else 0.0 for content in contents]
...
def get_reward_funcs(script_args) -> list[Callable]:
    REWARD_FUNCS_REGISTRY = {
        ...
        "material_accuracy": material_accuracy_reward,
        "material_format": material_format_reward,
